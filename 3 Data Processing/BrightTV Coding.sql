...
select * from `brightv`.`tv`.`bright_tv_dataset_4` limit 100;

select * from `brightv`.`tv`.`bright_tv_dataset_4`;
...
SELECT
 ...
 Province,
 `Social Media Handle`
FROM `brightv`.`tv`.`bright_tv_dataset_4`;
...
FROM `brightv`.`tv`.`bright_tv_dataset_3`
)
